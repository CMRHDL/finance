...
export const generateCode = ({ number, page, position, year }) => {
  return year + padStart(number, 3, '0') + padStart(page, 2, '0') + padStart(position, 2, '0')
}
export const parseCode = code => {
  const [, year, number, page, position] = /(\d{4})(\d{3})(\d{2})(\d{2})/g.exec(code).map(e => numeral(e).value())
